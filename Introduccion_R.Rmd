...
R/
De acuerdo a la ayuda el paquete base tiene las funciones basicas aritmeticas
...
```{R inputExample}
source("C:/Users/94431200/Documents/GitHub/Hello_word/example2.R")
```
```{R outputExample, echo=FALSE}
source("C:/Users/94431200/Documents/GitHub/Hello_word/example3.R")
```
...
R/abs (valor absoluto de x)
sqrt (raiz cuadrada de x)
...
a[2:6]
```
R1/ a[3] me retorna la posicion 3 del vector a

R2/a[c(1, 3, 5)] me retorna las posiciones 1, 3 y 5 del vector

R3/a[2:6] me retorna las posiciones desde la 2 hasta la 6
...
# -------------------------
```
colocando byrow en TRUE le estoy diciendo que empiece a llenar la matrix por filas
...
```{R matrizExample3_1}
x[ , 2] <- 50 #cambio todos los elementos de la columna 2 por 50
x
x[1, ] #obtenemos los elementos de la primera fila
x
x[5 , 3] <- 30 #cambio el elementos de la posicion 5 columna 3 por 30
x
```
...
```{r attachExample}
summary(mtcars$mpg) #resumen los calculos estadisticos de la columna mpg de la base de datos mtcards
...
```{R factorExample3}
xfactor <- factor(c("yes", "yes", "no", "yes", "no"), levels = c("yes", "no"))
```
_Ejercicio 10_
...
R/LOS NIVELES SON 1,2,3,4,5, Y NA
...
R/EL COMANDO A EJECUTAR ES EL C
...
parks <- read.csv("C:/Users/94431200/Documents/GitHub/Hello_word/parks.csv")
...
```
CON EDIT ME SALE UNA NUEVA PANTALLA CON EL DATAFRAME
